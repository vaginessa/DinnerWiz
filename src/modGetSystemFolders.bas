...
Public Sub getSystemFolders(mySysFolders As systemFolders)
    ...
    If ret <> "" Then   'shfolder excists, so use it!
        ...
        mySysFolders.AppData = shGetFolder(CSIDL_COMMON_APPDATA)
        If Right(mySysFolders.AppData, 1) <> "\" Then mySysFolders.AppData = mySysFolders.AppData & "\"
    Else
        mySysFolders.ProgramFiles = "c:\Program Files\" 'set default
        If mySysFolders.AppData = "" Then mySysFolders.AppData = App.Path
    End If
